<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="page-title">Edit Target Hafalan</h2>
            <div class="text-secondary mt-1">Perbarui target hafalan untuk {{ $target->santri?->full_name ?? '-' }}.</div>
        </div>
    </x-slot>

    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('tahfidz.targets.update', $target) }}" class="row g-3">
                @csrf
                @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <select name="santri_id" class="form-select @error('santri_id') is-invalid @enderror" required>
                        ...
                        @foreach ($santriOptions as $santri)
                            <option value="{{ $santri->id }}" @selected(old('santri_id', $target->santri_id) == $santri->id)>
                ...
                <div class="col-md-3">
                    ...
                    <select name="target_type" class="form-select @error('target_type') is-invalid @enderror" required>
                        ...
                        @foreach ($typeOptions as $opt)
                            <option value="{{ $opt['value'] }}" @selected(old('target_type', $target->target_type) === $opt['value'])>
                                {{ $opt['label'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="target_value" class="form-control @error('target_value') is-invalid @enderror"
                           value="{{ old('target_value', $target->target_value) }}" min="1" max="99999" required>
                    @error('target_value')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="target_date" class="form-control @error('target_date') is-invalid @enderror"
                           value="{{ old('target_date', $target->target_date?->toDateString()) }}">
                    @error('target_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6">
                    ...
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="2"
                              maxlength="1000">{{ old('notes', $target->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
