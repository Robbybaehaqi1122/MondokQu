<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="page-title">Edit Jadwal Setoran</h2>
            <div class="text-secondary mt-1">Perbarui jadwal setoran hafalan musyrif/ustadz.</div>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('tahfidz.jadwal.update', $schedule) }}">
        @csrf
        @method('PUT')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select id="musyrif_id" name="musyrif_id" class="form-select @error('musyrif_id') is-invalid @enderror" required>
                                ...
                                @foreach ($musyrifOptions as $musyrif)
                                    <option value="{{ $musyrif->id }}" @selected(old('musyrif_id', $schedule->musyrif_id) == $musyrif->id)>
                                        {{ $musyrif->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="time" id="start_time" name="start_time" class="form-control @error('start_time') is-invalid @enderror"
                                    value="{{ old('start_time', $schedule->start_time ? \Carbon\Carbon::parse($schedule->start_time)->format('H:i') : '') }}" required>
                                @error('start_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="time" id="end_time" name="end_time" class="form-control @error('end_time') is-invalid @enderror"
                                    value="{{ old('end_time', $schedule->end_time ? \Carbon\Carbon::parse($schedule->end_time)->format('H:i') : '') }}" required>
                                @error('end_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" id="max_santri" name="max_santri" class="form-control @error('max_santri') is-invalid @enderror"
                                    value="{{ old('max_santri', $schedule->max_santri) }}" min="1" max="255" required>
                                @error('max_santri')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-0">
                            ...
                            <textarea id="notes" name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="Catatan tambahan tentang jadwal ini...">{{ old('notes', $schedule->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
