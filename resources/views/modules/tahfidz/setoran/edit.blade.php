<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="page-title">Edit Setoran Hafalan</h2>
            <div class="text-secondary mt-1">Perbarui setoran hafalan Al-Quran untuk {{ $session->santri?->full_name ?? '-' }}.</div>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('tahfidz.setoran.update', $session) }}" id="setoranForm">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <div class="form-control-plaintext fw-semibold">
                                {{ $session->santri?->full_name ?? '-' }} (NIS {{ $session->santri?->nis ?? '-' }})
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" id="session_date" name="session_date" class="form-control @error('session_date') is-invalid @enderror"
                                value="{{ old('session_date', $session->session_date?->toDateString()) }}" required>
                            @error('session_date')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="notes" name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="Catatan tambahan tentang setoran ini...">{{ old('notes', $session->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-0">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="status_completed" value="completed" @checked(old('status', $session->status) === 'completed')>
                                <label class="form-check-label" for="status_completed">Selesai</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="status_draft" value="draft" @checked(old('status', $session->status) === 'draft')>
                    ...
                    <div class="card-header">
                        <div class="d-flex align-items-center justify-content-between w-100">
                            <div>
                                ...
                                <div class="text-secondary small mt-1">Sesuaikan ayat yang disetorkan dalam sesi ini.</div>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        let recordIndex = {{ old('records') ? count(old('records')) : $session->records->count() }};
