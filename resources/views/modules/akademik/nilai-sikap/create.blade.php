<x-app-layout>
    <x-slot name="header">
        <div class="d-flex align-items-center justify-content-between">
            <div>
                <h2 class="page-title">Input Nilai Sikap</h2>
                <div class="text-secondary mt-1">Catat penilaian sikap spiritual & sosial santri</div>
            </div>
    ...
    <form action="{{ route('akademik.nilai-sikap.store') }}" method="POST">
        @csrf
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="sikap_spiritual" class="form-select">
                                ...
                                <option value="SB" {{ old('sikap_spiritual') === 'SB' ? 'selected' : '' }}>SB - Sangat Baik</option>
                                <option value="B" {{ old('sikap_spiritual') === 'B' ? 'selected' : '' }}>B - Baik</option>
                                <option value="C" {{ old('sikap_spiritual') === 'C' ? 'selected' : '' }}>C - Cukup</option>
                                <option value="K" {{ old('sikap_spiritual') === 'K' ? 'selected' : '' }}>K - Kurang</option>
                            </select>
                        </div>
                        <div>
                            <label class="form-label">Deskripsi / Uraian</label>
                            <textarea name="deskripsi_spiritual" class="form-control" rows="3"
                                placeholder="Uraian penilaian sikap spiritual...">{{ old('deskripsi_spiritual') }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="sikap_sosial" class="form-select">
                                <option value="">-- Pilih Predikat --</option>
                                <option value="SB" {{ old('sikap_sosial') === 'SB' ? 'selected' : '' }}>SB - Sangat Baik</option>
                                <option value="B" {{ old('sikap_sosial') === 'B' ? 'selected' : '' }}>B - Baik</option>
                                <option value="C" {{ old('sikap_sosial') === 'C' ? 'selected' : '' }}>C - Cukup</option>
                                <option value="K" {{ old('sikap_sosial') === 'K' ? 'selected' : '' }}>K - Kurang</option>
                            </select>
                        </div>
                        <div>
                            <label class="form-label">Deskripsi / Uraian</label>
                            <textarea name="deskripsi_sosial" class="form-control" rows="3"
                                placeholder="Uraian penilaian sikap sosial...">{{ old('deskripsi_sosial') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <textarea name="catatan_wali" class="form-control" rows="3"
                            placeholder="Catatan tambahan dari wali kelas (opsional)...">{{ old('catatan_wali') }}</textarea>
                    </div>
                </div>
            </div>
